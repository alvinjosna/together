(function(){
  ...
  function unlockSite(guestPayload, codeUsed){
    ...
    // Start cinematic scroll effects (fade-in + parallax)
  ...
  // ------------------------------------------------------------
  // Cinematic animations (fade-in / slide-up / stagger + parallax)
  // ------------------------------------------------------------
  let __cinematicStarted = false;

  function startCinematicEffects(){
    if(__cinematicStarted) return;
    __cinematicStarted = true;

    // Add a hook class so CSS can apply small delays
    ...
    // ------- Scroll reveal -------
    const root = document.getElementById('pageFrame') || document.body;

    const setReveal = (el, delay = 0, variant = '') => {
      if(!el) return;
    ...
    // Core elements
    setReveal(root.querySelector('.heroT__date'), 40, 'reveal--fade');
    setReveal(root.querySelector('.heroT__tagline'), 120, 'reveal--fade');
    setReveal(root.querySelector('.heroT__greeting'), 200, 'reveal--fade');

    // Details stack
    root.querySelectorAll('#details .card--simple').forEach((el, i) => setReveal(el, 80 + i * 120));
    setReveal(root.querySelector('#details .bibleInline'), 220, 'reveal--fade');

    // Bible separators
    root.querySelectorAll('.bibleSep__text').forEach((el) => setReveal(el, 0, 'reveal--fade'));

    // Schedule banner
    setReveal(root.querySelector('#schedule .banner__kicker'), 40, 'reveal--fade');
    setReveal(root.querySelector('#schedule .banner__title'), 120);
    root.querySelectorAll('#schedule .time').forEach((el, i) => setReveal(el, 180 + i * 110));

    // Location
    setReveal(root.querySelector('#location .locationT__title'), 40, 'reveal--fade');
    setReveal(root.querySelector('#location .locationT__left'), 140);
    setReveal(root.querySelector('#location .locationT__right'), 260);

    // Countdown
    setReveal(root.querySelector('.countBanner .banner__kicker'), 40, 'reveal--fade');
    root.querySelectorAll('.countBanner .cb').forEach((el, i) => setReveal(el, 140 + i * 90));

    // Cards (Photos/Travel/People/Wishes etc.)
    root.querySelectorAll('.twoCards .card, .cards .card').forEach((el, i) => setReveal(el, 60 + (i % 6) * 80));
    setReveal(root.querySelector('#wishes .h2'), 40, 'reveal--fade');
    setReveal(root.querySelector('#wishes form'), 140);

    // Footer
    root.querySelectorAll('footer .foot__grid > *').forEach((el, i) => setReveal(el, 120 + i * 80));

    const allReveal = Array.from(root.querySelectorAll('.reveal'));
    ...
    const parallaxItems = [
      { el: document.querySelector('.banner__bg'), speed: 0.08, scale: 1.22 },
      { el: document.querySelector('.countBanner__bg'), speed: 0.06, scale: 1.18 }
    ].filter(x => x.el);
    ...
    if(parallaxItems.length){
      ...
      const update = () => {
        ...
        parallaxItems.forEach(item => {
          ...
          const progress = (center - viewportH / 2) / (viewportH / 2);
          const offset = -progress * 24; // max ~ +/- 24px

          item.el.style.transform = `translate3d(0, ${offset.toFixed(2)}px, 0) scale(${item.scale})`;
        });
      };
